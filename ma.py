...
st.markdown("Enter movie details below to prdict the rating!")

actors=st.text_input("Enters Actors")
directors=st.text_input("Enters Directors")
genres=st.text_input("Enter the movie synopsis")

if st.button("Predict Rating"):
    with st.spinner("Predicting...."):
        try:
            input_actor_avg=np.mean([avg_actor_rating.get(actor.strip(),np.nan)for actor in actors.split(",")])
            input_director_avg=np.mean([avg_director_rating.get(directors.strip(),np.nan)for director in directors.split(",")])
            ...
            for genre in input_genres:
                if genre in genres_columns:
                    ...
                    predict_rating=xgb_model.predict(features)[0]
                    st.success(f"Predict Rating: {predict_rating:.2f} / 10")
